  ...
  return (
    <div className="w-2/3 flex justify-center gap-6">
      {timer.status !== "off" ?
        ...
        >
          <i className="text-5xl ri-loop-left-fill" />
        </button> : null}
      ...
      >
        <i className={`text-6xl ri-${timer.status === "off" ? "play" : timer.status === "on" ? "pause" : "play"}-circle-fill`} />
      </button>
      ...
      {timer.status !== "off" ?
        <button className="hover:cursor-pointer hover:scale-110 dark:text-gray-300 transition-all duraion-200 ease" onClick={() => updateTimerStatus("off")}>
          <i className="text-6xl ri-stop-circle-line" />
        </button> : null}
    </div>
  )
}